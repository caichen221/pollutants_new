...
import com.iscas.common.prometheus.tools.model.targets.TargetResponse;
...
public class PrometheusServiceTest {
    ...
    private RefreshRuleService refreshRuleService;

    @BeforeEach
    public void before() {
        prometheusService = new PrometheusService(new ConnectionCfg(), new CustomHttpClient(new CustomHttpClient.HttpClientProps()));
        refreshRuleService = new RefreshRuleService(true, "D:\\soft\\prometheus\\prometheus-2.37.6.windows-amd64", prometheusService);
    }
     ...
     */
    @Test
    public void createAlert() throws PrometheusException, IOException {
        ...
        rule.setLabels(new ArrayList<>(){{
            add(new Label("severity", "emergency"));
        }});
        rule.setType("alerting");
        rule.setAnnotations(new ArrayList<>(){{
            add(new Annotation("description", "战位: {{ $labels.host_address }},CPU利用率1分钟内大于30%,当前值：{{ $value }}%"));
        ...
        Rule rule2 = new Rule();
        rule2.setName("station-exporter-cpu-high-alert-test2");
        rule2.setQuery("(station_system_cpu_used_C{collector_name=\"station_system_cpu_used\"} / ignoring(collector_name) station_system_cpu_total_C) * 100 > 20");
        rule2.setDuration(10);
        rule2.setLabels(new ArrayList<>(){{
            add(new Label("severity", "emergency"));
        }});
        rule2.setType("alerting");
        rule2.setAnnotations(new ArrayList<>(){{
            add(new Annotation("description", "战位: {{ $labels.host_address }},CPU利用率1分钟内大于20%,当前值：{{ $value }}%"));
            add(new Annotation("instance", "{{ $labels.instance }}"));
            add(new Annotation("stationName", "{{ $labels.station_name }}"));
            add(new Annotation("summary", "战位: {{ $labels.host_address }},CPU利用率10秒内大于20%"));
            add(new Annotation("value", "{{ $value }}"));
        }});


        group.setRules(List.of(rule, rule2));
        group.setFile("test-rule.yml");

        groups.add(group);
        refreshRuleService.createOrUpdateRule(groups);
     ...
     */
    @Test
    public void deleteAlert() throws PrometheusException, IOException {
        List<Group> groups = new ArrayList<>();
        Group group = new Group();
        group.setName("station-exporter-cpu-alert-test");
        Rule rule = new Rule();
        rule.setName("station-exporter-cpu-high-alert-test2");
        group.setRules(List.of(rule));
        group.setFile("test-rule.yml");
        groups.add(group);
        refreshRuleService.deleteRule(groups);
    }
