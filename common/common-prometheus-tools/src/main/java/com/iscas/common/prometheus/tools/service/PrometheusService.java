...
import com.iscas.common.prometheus.tools.model.targets.Target;
    ...
    private QueryResponse<? extends Result> analysisResult(String resBody) throws PrometheusException {
        ...
        if (Objects.equals(VECTOR, resultTypeStr)) {
            ...
            List<MatrixResult> matrixResults = resultMap.stream().map(rm -> {
                ...
                // 生成value对象数组
                List<Value> values = valueList.stream().map(list -> new Value((Double) list.get(0), (String) list.get(1))).collect(Collectors.toList());
