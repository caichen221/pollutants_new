 ...
 */
@Deprecated
public class BPlusTree_old<T, V extends Comparable<V>>{
    //B+树的阶
    ...
    //无参构造方法，默认阶为3
    public BPlusTree_old(){
        this(3);
    }

    //有参构造方法，可以设定B+树的阶
    public BPlusTree_old(Integer bTreeOrder){
    ...
    //插入
    public void put(T value, V key){
        if(key == null) {
            return;
        }
        Node<T, V> t = this.root.put(value, key);
        if(t != null) {
            this.root = t;
        }
...
//        System.out.println();
    ...
    abstract class Node<T, V extends Comparable<V>>{
        ...
        //插入
        abstract Node<T, V> put(T value, V key);
...
//            System.out.println("非叶子节点查找key: " + this.keys[i]);
            ...
            return this.childs[i].put(value, key);
        }
